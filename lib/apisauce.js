...
import { cond, isNil, identity, is, T, curry, gte, anyPass, isEmpty, ifElse, complement, has, prop, propIs, propSatisfies, merge, dissoc, keys, pipeP, partial, contains, always, equals } from 'ramda';
/**
...
var NODEJS_CONNECTION_ERROR_CODE_NF = 'ENOTFOUND';
var NODEJS_CONNECTION_ERROR_CODE_REF = 'ECONNREFUSED';
var NODEJS_CONNECTION_ERROR_CODE_RES = 'ECONNRESET';
...
export var create = function (config) {
    ...
    var addAsyncResponseTransform = function (transform) {
        return asyncResponseTransforms.push(transform);
    };
    ...
    var setHeaders = function (headers) {
        keys(headers).forEach(function (header) { return setHeader(header, headers[header]); });
        // forEach()
        return instance;
    };
    // remove header
            ...
            switch (_a.label) {
                case 0:
                    ...
                    if (requestTransforms.length > 0) {
                        ...
                        requestTransforms.forEach(function (transform) { return transform(axiosRequestConfig); });
                    }
                ...
                case 5:
                    ...
                    return [3 /*break*/, 1];
                case 6:
                    chain = pipeP(convertResponse(toNumber(new Date())), 
     ...
     */
    var convertResponse = curry(function (startedAt, axiosResult) { return __awaiter(_this, void 0, void 0, function () {
        var end, duration, isError, axiosResponse, axiosError, response, status, problem, ok, config, headers, data, transformedResponse, index, transform;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    end = toNumber(new Date());
                    duration = end - startedAt;
                    isError = axiosResult instanceof Error || axios.isCancel(axiosResult);
                    axiosResponse = axiosResult;
                    axiosError = axiosResult;
                    response = isError ? axiosError.response : axiosResponse;
                    status = (response && response.status) || null;
                    problem = isError
                        ? getProblemFromError(axiosResult)
                        : getProblemFromStatus(status);
                    ok = in200s(status);
                    config = axiosResult.config || null;
                    ...
                    if (responseTransforms.length > 0) {
                        responseTransforms.forEach(function (transform) { return transform(transformedResponse); });
                    }
                    if (!(asyncResponseTransforms.length > 0)) return [3 /*break*/, 6];
                    index = 0;
                    _a.label = 1;
                case 1:
                    if (!(index < asyncResponseTransforms.length)) return [3 /*break*/, 6];
                    transform = asyncResponseTransforms[index](transformedResponse);
                    if (!isPromise(transform)) return [3 /*break*/, 3];
                    return [4 /*yield*/, transform];
                case 2:
                    _a.sent();
                    return [3 /*break*/, 5];
                case 3: return [4 /*yield*/, transform(transformedResponse)];
                case 4:
                    _a.sent();
                    _a.label = 5;
                case 5:
                    index++;
                    return [3 /*break*/, 1];
                case 6: return [2 /*return*/, transformedResponse];
            }
        });
    }); });
    /**
    ...
    var sauce = {
        ...
        addAsyncResponseTransform: addAsyncResponseTransform,
        setHeader: setHeader,
